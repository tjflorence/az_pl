...
for ii = 1:length(ol_a_files)
   ...
   sync_exist = ~isempty(dir([idir dash 'sync_' num2str(b_num, '%03d')]));
   if sync_exist
       try
   ...
   % throw off sync
   load([bdir dash ol_a_files(ii).name])
   ...
   if sync_exist == 1 && i_exist == 1 && no_berror == 1
           ...
           sum_struct.fpaths(b_num).bpath = [bdir dash ol_a_files(ii).name];
           sum_struct.fpaths(b_num).ipath = [idir  dash 'trial_' num2str(b_num, '%03d')];
           sum_struct.fpaths(b_num).spath = [idir  dash 'sync_' num2str(b_num, '%03d')];
...
for ii = 1:length(bsort)
   ...
   if sync_exist == 1 && i_exist == 1 && no_berror == 1
           ...
           sum_struct.fpaths(b_num).bpath = [bdir dash bsort(ii).name];
           sum_struct.fpaths(b_num).ipath = [idir dash 'trial_' num2str(b_num, '%03d')];
           sum_struct.fpaths(b_num).spath = [idir dash 'sync_' num2str(b_num, '%03d')];
...
for ii = 1:length(ol_b_files)
   ...
   %% checks for behavior files -- sometime read errors from ball
   % throw off sync
   load([bdir dash ol_b_files(ii).name])
   ...
   if sync_exist == 1 && i_exist == 1 && no_berror == 1
           ...
           sum_struct.fpaths(b_num).bpath = [bdir dash ol_b_files(ii).name];
           sum_struct.fpaths(b_num).ipath = [idir dash 'trial_' num2str(b_num, '%03d')];
           sum_struct.fpaths(b_num).spath = [idir dash 'sync_' num2str(b_num, '%03d')];
