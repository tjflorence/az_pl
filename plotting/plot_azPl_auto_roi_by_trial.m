function plot_azPL_auto_roi_by_trial(expdir, expver)
...
train_heat_env_map = [-2*ones(96, expr.c_trial.dark_time*dilate_factor) ...
...
%% create plotp
for kk = 1:length(roi_auto_struct)
...
s3 = subplot(4,3,1:2);
imagesc(mean(expr.c_trial.idata.df_frames, 3));
colormap(gray)
axis equal off tight
hold on

scat_h = fill(roi_auto_struct(kk).xy(:,1), roi_auto_struct(kk).xy(:,2), roi_auto_struct(kk).cmap);
...
plot(x_vals, y_vals, 'color', roi_auto_struct(kk).cmap, 'linewidth', 2);
hold on
...
vss_yvals = conv(expr.c_trial.bdata.vss(1:expr.c_trial.bdata.count), ones(25,1)/25, 'same');
...
plot(x_vals, vss_yvals, 'color', 'b', 'linewidth', 1.5);
...
if expver == 1
    ...
    text(70, .5, 'vSS', 'Color', 'b', 'Fontsize', 25)
...
elseif expver == 2
    ...
    text(65, .5, 'vSS', 'Color', 'b', 'Fontsize', 25)
end
xlabel('time (sec)')
ylabel('ball motion\newline     (AU)')
...
print(f1, ['trace_by_trial_autoROI_' num2str(kk, '%03d') '_trial_' num2str(expr.c_trial.rep, '%03d') '.pdf'],...
            '-dpdf', '-r0', '-opengl');
