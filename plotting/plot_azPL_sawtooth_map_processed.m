function plot_azPL_sawtooth_map_processed(expdir)
...
%% here extract response as a fcn of position
    ...
    std_ang_resp_a(1) = nanstd(expr.c_trial.idata.auto_roi_traces(c_roi, resp_1))/sqrt(numel(resp_1));
    ...
    for ii = 2:8
        ...
        std_ang_resp_a(ii) = nanstd(expr.c_trial.idata.auto_roi_traces(c_roi, resp_dF))/sqrt(numel(resp_dF));
    ...
    std_ang_resp_b(1) = nanstd(expr.c_trial.idata.auto_roi_traces(c_roi, resp_1))/sqrt(numel(resp_dF));
    ...
    for ii = 2:8
        ...
        std_ang_resp_b(1,ii) = nanstd(expr.c_trial.idata.auto_roi_traces(c_roi, resp_dF))/sqrt(numel(resp_dF));

    end

    mean_ang_resp_a = circshift(mean_ang_resp_a, [0 4]);
    std_ang_resp_a = circshift(std_ang_resp_a, [0 4]);

    mean_ang_resp_b = circshift(mean_ang_resp_b, [0 4]);
    std_ang_resp_b = circshift(std_ang_resp_b, [0 4]);
...
%% cd and print
    ...
    fig_name = ['OL_map_processed_ROI_' num2str(c_roi, '%03d')];
