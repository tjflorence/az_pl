...
for ii = 1:length(cl)
    ...
    cl_vals(ii,1) = summary_data.norm_out(1) ;
    cl_vals(ii,2) = summary_data.norm_out(2) ;
...
for ii = 1:length(hcl)
    ...
    hcl_vals(ii,1) = summary_data.norm_out(1) ;
    hcl_vals(ii,2) = summary_data.norm_out(2) ;
...
for ii = 1:length(ucl)
    ...
    ucl_vals(ii,1) = summary_data.norm_out(1) ;
    ucl_vals(ii,2) = summary_data.norm_out(2) ;
...
prettyprint(f1,'learning_measure_normout')
close all
