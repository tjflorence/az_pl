...
for c_exp = 1:length(dark_type);
    cd(dark_type(c_exp).path)

    c_roi = dark_type(c_exp).roi;
    ...
    for ii = 1:length(pulse_30)
        ...
        if isfield(expr.c_trial, 'idata')
            if expr.c_trial.viz_type == 0
                ...
                dark_type_summary(c_exp).dark_data{dark_trial} = ...
                expr.c_trial.idata.auto_roi_traces(c_roi,:);
            else
                light_trial = light_trial+1;
                dark_type_summary(c_exp).light_data{light_trial} = ...
                expr.c_trial.idata.auto_roi_traces(c_roi,:); 
            end
        end
...
for c_exp = 1:length(dark_type);
    ...
    dark_interp = nan(numel(dark_type_summary(c_exp).dark_data), tlen);
    light_interp = nan(numel(dark_type_summary(c_exp).light_data), tlen);

    for ii = 1:numel(dark_type_summary(c_exp).dark_data)
       
        c_data = dark_type_summary(c_exp).dark_data{ii};
        dark_interp(ii,:) = spline(1:length(c_data), c_data,...
                                linspace(1, length(c_data), tlen ));
        
    end

    for ii = 1:numel(dark_type_summary(c_exp).light_data)
       
        c_data = dark_type_summary(c_exp).light_data{ii};
        light_interp(ii,:) = spline(1:length(c_data), c_data,...
                                linspace(1, length(c_data), tlen ));
        
    end
    
    dark_type_summary(c_exp).dark_mean = mean(dark_interp, 1);
    dark_type_summary(c_exp).light_mean = mean(light_interp, 1);
...
cd(print_dir)
prettyprint(f1, 'dark_type_response')