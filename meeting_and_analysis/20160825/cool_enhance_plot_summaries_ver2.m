...
hot_blind(1).path = '\\reiser_nas\tj\az_pl\processed\20160825190814_11f03_OL_stim';
hot_blind(1).roi = 2;
hot_blind(1).ver = 2;

hot_blind(2).path = '\\reiser_nas\tj\az_pl\processed\20160825145626_11f03_OL_stim';
hot_blind(2).roi = 2;
hot_blind(2).ver = 2;

hot_blind(3).path = '\\reiser_nas\tj\az_pl\processed\20160818212641_11f03_OL_stim';
hot_blind(3).roi = 2;
hot_blind(3).ver = 2;

hot_blind(4).path = '\\reiser_nas\tj\az_pl\processed\20160823112630_11f03_OL_stim';
hot_blind(4).roi = 2;
hot_blind(4).ver = 2;

%% information about plotting groups
plot_collections(1).name = 'refVIZ_testCOOL';
plot_collections(1).conditions_v2 = [3 6 9 12 15];

plot_collections(2).name = 'refCOOL_testVIZ';
plot_collections(2).conditions_v2 = [26 27 28 29 30];

plot_collections(3).name = 'refVIZ_testNOHEAT';
plot_collections(3).conditions_v1 = [1 3 5 7 9 ];
plot_collections(3).conditions_v2 = [1 4 7 10 13];

plot_collections(4).name = 'refVIZ_testHEAT';
plot_collections(4).conditions_v1 = [2 4 6 8 10];
plot_collections(4).conditions_v2 = [2 5 8 11 14];

plot_collections(5).name = 'refNOHEAT_testVIZ';
plot_collections(5).conditions_v1 = [11 12 13 14 15];
plot_collections(5).conditions_v2 = [16 17 18 19 20];

plot_collections(6).name = 'refHEAT_testVIZ';
plot_collections(6).conditions_v1 = [16 17 18 19 20];
plot_collections(6).conditions_v2 = [21 22 23 24 25];
        ...
        for jj = 1:length(plot_collections(ii).conditions_v1)
            ...
            if hot_blind(bb).ver == 1
                ...
                if bb > 1
                    ...
                    plot_collections(ii).trial_data(jj).mean_dF = plot_collections(ii).trial_data(jj).mean_dF - mean(plot_collections(ii).trial_data(jj).mean_dF(300:900));
                end
...
for collection_num = 1:length(plot_collections);
    ...
    fig_name = ['multiexp_static_summary_group_' plot_collections(collection_num).name];
