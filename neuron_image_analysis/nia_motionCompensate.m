function [found_displ, export_ref_im] = nia_motionCompensate(mov, pos_vec, home_pos, ...
    pos_idx, channel_id, window, fixed_ref_im)
...
%           used.
%
%       ref_alg - An optional argument that specifies the algorithm to use
%           for calculating the reference image. Options are 'iter' and
%           'fixed'. The 'iter' algorithm calculates the reference image
%           by finding a mean image over a small period of time, shifting
%           the images within that period to align with the mean image,
%           and repeating. The period of time is incrementally increased
%           until the period of time spans the entire movie. This algorithm
%           allows for the construction of an accurate reference image even
%           when there is substantial motion. The 'fixed' algorithm aligns
%           each frame to a fixed reference image passed in the ref_udata
%           argument.
%
%       ref_udata - An optional argument that contains additional data for
%           a particular reference image algorithm.
%           
%
...
if cfg.enable_multipass
    ...
    ref_widths_list = ceil(ref_widths_list);
else
