function auto_process_roi(expdir)
...
if isfield(expr.c_trial, 'idata')
    for ii = 4:(frame_y-3)
        ...
        %    var_map(:,:,c_trial) = iqr(df_frames, 3);
            rawF_map(:,:,c_trial) = max(expr.c_trial.idata.mcorr_MIP,[], 3);
        end
    end
end
...
f1 = figure;
mean_dot_map = max(dot_map,[], 3);
%mean_var_map = mean(var_map, 3);
mean_rawF = max(rawF_map,[], 3);
...
while accept ~= 1
    ...
    thresh_img(mean_rawF>thresh_bg) = 1;
...
while accept ~= 1
    ...
    thresh_img(mean_rawF<=thresh_bg) = 0;
